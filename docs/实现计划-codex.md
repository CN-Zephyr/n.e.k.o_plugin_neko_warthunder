...
- 正式 pytest 基线为 `536 passed`。`tests/run_logic_tests.py` 已支持当前简单 `pytest.mark.parametrize` 隔离用例，一键逻辑自检与 pytest 统一为 `536/536`。
...
- 最后一次完整 pytest 基线为 `536 passed`；逻辑自检为 `536/536 passed`。播报偏好、频率设置和安全活动中心已完成离线实现，数据层 P0、UI 隔离、package artifact gate 和统一 RC 构建演练已有通过记录；聚焦真机 dry-run 按当前决定延期。
