...
- 数据库/profile 已完成三批 Datamine 候选复核并入库：J-15T/Su-30/F-16/F/A-18/Mirage/Mirage 2000/A6M、F-14/F-15/MiG-29/Su-27/Su-33/J-10/J-11/Rafale/JAS39/Eurofighter、F-5/MiG-21/MiG-23/F-104/A-10/Su-25/Tornado/Kfir/Harrier/AV-8/Sea Harrier/Yak-141。`tools/datamine_profile_candidates.py` 只读抽取候选，`tools/profile_candidate_diff.py` 对比当前 `vehicle_profiles.json`；报告写入 ignored 的 `local_test_logs/`，不提交素材或报告。
- recovery 暂缓。
...
- 过热/炸缸真机 smoke 中，游戏 UI 已出现油温/发动机异常；插件侧已补 `hud_notices.feed[].code=engine_overheat/oil_overheat` 到 `overheat` 的映射，2026-06-23 已观察到 `overheat` dry_run 基础链路。Datamine 热模型字段目前只作为 evidence，不直接生成 HUD 过热 warn/critical 阈值；油温/发动机细项仍需真机样本校准后再细分。`powertrain_failure` 暂不直接提升为播报事件，但会以 `detector_suppressed/deferred_hud_notice` 记录到 T-Observe / live monitor，方便现场判断“识别到了但当前策略不播”。
