...
def test_missing_python_runner_uses_embedded_fallback():
    ...
    checks = iter([False, True])
    proc = FakeProcess()
    embedded_calls: list[tuple[Path, int]] = []

    def fake_embedded(data_process_dir: Path, *, port: int):
        embedded_calls.append((data_process_dir, port))
        return proc
    ...
    assert embedded_calls[0][1] == 8112
    assert proc.terminated is True
